...
export default function NewItem({categoryState,  setNewItemState, setHidden}) {
  const [title, setTitle] = useState('');
  const [contents, setContents] = useState('');

  const n = () => {
    setNewItemState({contents: title, description: contents, categoryId: categoryState.id});
    setHidden(true)
    reset();
  }
  ...
  return (<div className='new-category'>
    ...
    >
      ...
      <div
        className='nc-contents'
        style={{
          display: 'flex', justifyContent: 'space-between', marginBottom: 12
        }}
      >
        <span style={{paddingTop: 4, fontSize: 18}}>Contents</span>
        <textarea
          className='l-input'
          type='text'
